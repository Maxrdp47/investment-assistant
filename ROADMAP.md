...
**Programm-Stopp am 2026-09-22:** R1 ist terminal unterpowert. R2 kann wegen fehlender belegbarer Futures-Kontrakt-/Roll-/Open-Provenienz unter den derzeit freigegebenen Quellen nicht fachlich gültig gestartet werden. Der Gesamtzustand ist `RESEARCH_PROGRAM_BLOCKED_BY_DATA_OR_TECHNICAL_LIMIT_REQUIRES_REVIEW`; R3–R9 bleiben geschlossen. Siehe [technischen Abschluss](FINITE_RESEARCH_PROGRAM_TECHNICAL_STOP_2026-09-22.md). Diese Daten-Gate-Entscheidung ist keine Behauptung eines allgemeinen No-Edge-Ergebnisses.
...
R3–R9 sind infolge des technischen Gesamtstopps nicht freigegeben, auch wenn ihre Tabelle die ursprüngliche Reihenfolge dokumentiert. Kein anderer `READY`-Work-Request und keine außerhalb dieser Tabelle liegende Idee darf als Ersatz ausgewählt werden. Autorisierte signalunabhängige Collector dürfen weiterlaufen, sofern sie keinen echten Prozess-, Lock-, DB- oder Ressourcen-Konflikt verursachen.
