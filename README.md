...
Bereits vollständig geprüfte tradingrelevante Claims aus ENTRY können ohne Bestätigungsdialog über `python scripts/import_entry_handoff.py --input <handoff.json> --json-output` importiert werden. Der transaktionale Import ist idempotent, schützt lokale Änderungen über ein Konfliktprotokoll und legt Testideen höchstens als `DRAFT` an. Das vollständige Schema, Mapping, Exit-Codes und ein Dry-Run stehen in [docs/entry_handoff_import.md](docs/entry_handoff_import.md).
...
Der Video-Inbox ist domainübergreifend: Eine Source wird global nur einmal gespeichert, anschließend werden ihre Claims einzeln klassifiziert. Stabile Hauptdomänen sind `TRADING_INVESTMENT`, `IMMOBILIEN`, `STEUERN`, `UNTERNEHMERTUM`, `PERSONAL_FINANCE`, `AI_TECH`, `OTHER` und optional `CROSS_DOMAIN`. Jeder Claim besitzt eine primäre Domain, optionale sekundäre Domains und Unterkategorie, eine begründete Trading-Relevanz sowie eine append-only Klassifikationshistorie. Ein erneuter Upload stoppt als `DUPLICATE_SOURCE`; nur bei einer bewusst vertieften Neuprüfung dürfen bislang fehlende Claims oder Domain-Einordnungen an derselben Source ergänzt werden.

Allgemeine Claims starten als `UNVERIFIED`. Weitere unveränderbare Prüfstände verwenden `SUPPORTED`, `PARTIALLY_SUPPORTED`, `CONFLICTING_EVIDENCE`, `INSUFFICIENT_EVIDENCE`, `REFUTED` oder `OUTDATED` und halten Confidence, Evidenzstärke, verifizierende Quellen, Gegenbelege, Einschränkungen und Prüfzeit fest. Für zeitabhängiges Steuer-, Rechts- oder Regulierungswissen können Jurisdiktion, Gültigkeitszeitraum, `valid_as_of` und Aktualisierungspflicht gespeichert werden. Ein TikTok allein bestätigt niemals eine Rechts- oder Steuerinformation.

Nur `TRADING_RELEVANT` und bewusst freigegebene `POTENTIALLY_TRADING_RELEVANT` Claims dürfen in den vorhandenen Hypothesen-, Experiment-, Work-Request- und Integrationspfad gelangen. Dies wird sowohl durch die Workflow-API als auch durch einen SQLite-Trigger erzwungen. Nicht-Trading-Claims erhalten keine Dummy-Hypothesen oder Experimente und können weder Trading-Scores noch Regeln oder Strategien beeinflussen. Ein gemischtes Video darf dennoch genau seinen separat klassifizierten Trading-Claim in den bestehenden Research-Pfad geben.

Der Domain-Export öffnet die KB im SQLite-Read-only-Modus, enthält keine Trading-Experimente oder Results und erzeugt eine stabile Import-ID sowie einen deterministischen Export-Fingerprint. JSON ist für eine spätere allgemeine Knowledge Base vorgesehen; Markdown dient der menschlichen Prüfung:

```powershell
.\.venv\Scripts\python.exe scripts\export_research_knowledge.py --domain IMMOBILIEN --format json
.\.venv\Scripts\python.exe scripts\export_research_knowledge.py --domain STEUERN --format markdown --verified-only
```
...
**Research Baseline != Production Strategy:** Next-Open-/Next-Bar-Entries, fixe Stops, fixe R-Ziele und 5/10/20/25-Sitzungs-Horizonte dienen in historischen Tests als kontrollierte Vergleichsbasis oder Counterfactual. Sie schreiben weder den späteren Produkteinstieg noch den späteren Exit oder eine maximale Haltedauer fest. Ein einzelnes technisch erfolgreiches Merkmal ist noch keine vollständige Produktionsstrategie.
...
- Legacy-Betrieb eingefroren: Die früheren regionalen Strategy-Forward-Aufgaben für Asien/Australien, Europa, Amerika/Global und Krypto sowie die alten Walk-Forward-/Kampagnenaufgaben sind deaktiviert. Auch ein manueller Hintergrundaufruf stoppt vor Markt- oder Datenbankarbeit. Die tägliche Prognoseaufgabe um 22:30 Uhr bleibt aktiv, überspringt aber die beiden früher nachgeschalteten America-/Krypto-Strategiestufen. Historische Daten bleiben vollständig erhalten; es werden niemals Orders ausgeführt.
