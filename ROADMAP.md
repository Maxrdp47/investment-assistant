...
**Programm am 2026-09-22 fortgesetzt:** R1 ist terminal unterpowert; R2 bleibt wegen fehlender belegbarer Futures-Kontrakt-/Roll-/Open-Provenienz ohne Research-Versuch blockiert. Der Nutzer hat mit „weiter“ eine erneute Fortsetzung beauftragt. Der Master-Vertrag sieht nach R2 ohne Holdout-PASS ausdrücklich R3 vor; die frühere Einordnung als *globaler* technischer Stop ist daher [als Zwischenstand überholt](FINITE_RESEARCH_PROGRAM_TECHNICAL_STOP_2026-09-22.md). Der [einzige R3-Versuch](OVERNIGHT_INTRADAY_R3_REVIEW_2026-09-22.md) ist ohne robusten Zusatznutzen abgeschlossen. Nächster zulässiger Schritt ist R4 Capability Expansion, nicht ein neuer Strategietest.
...
| R6 – Discovery v2 Development | `IMPLEMENTED_AWAITING_GREEN_CI_AND_PILOT` | R5 Freeze vollständig; R5-CI Smoke #60 grün; R6-Code muss für seinen exakten Commit noch CI sowie Integrity-/Replay-Pilot bestehen | Horizon-spezifische 20/60/120/252-Populationen; 31 vorab benannte Einzelfeatures; keine Massenkombination | 0 | Run `mad2-development-v2-20260922-v1`; neue append-only Delta-/Referenz-Stores; unveränderlicher v7-r2-Parent; Review-Vertrag `multi-asset-discovery-v2-r6-review-2026.09.22-v1` | vollständiger vorab definierter deskriptiver Review und kanonisches C-/Robustheitsgate | keine Validation während Discovery; kein Gesamt-Score; keine Top-Auswahl nach Profit; Dependency-Unknown nie durch Raw N ersetzen | vollständige lokale Prüfung → Commit/CI → deterministischer Drei-Asset-Pilot/Replay/Integrity → erst dann Full Development automatisch starten |
...
R4–R9 bleiben bis zum jeweiligen vorgelagerten Gate geschlossen. Kein anderer `READY`-Work-Request und keine außerhalb dieser Tabelle liegende Idee darf als Ersatz ausgewählt werden. Autorisierte signalunabhängige Collector dürfen weiterlaufen, sofern sie keinen echten Prozess-, Lock-, DB- oder Ressourcen-Konflikt verursachen.
