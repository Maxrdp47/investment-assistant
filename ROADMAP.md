...
- Der Discovery-v1-Vertrag ist inzwischen mit Contract-Fingerprint `68994e462f90e2a4f1ad4adbdc858cc43fb0ddb85b0c2662d0f647e1dfa6c05a` eingefroren und technisch implementiert. Ein elf Fälle umfassender Integritätspilot prüfte Aktien, ETFs, die drei Vertrags-FX-Paare, drei liquide Kryptos und einen Stage-Grenzfall. 18/19 Gates sind grün; der große Development-Scan bleibt weiterhin `NOT_STARTED`.
- Das verbleibende Gate ist ein echter FX-Datenqualitätsblocker: Im vorhandenen Store liegen 231 historische Balken, bei denen Open oder Close außerhalb der gemeldeten High-/Low-Hülle liegen; davon fallen 11 in die drei Pilot-Outcomes. Die Werte werden weder repariert noch imputiert. Daher lautet der verbindliche Status `NOT_READY_TO_START_MULTI_ASSET_DISCOVERY_V1_DEVELOPMENT`.
- Vor einem neuen Readiness-Pilot muss ein eigener vorab eingefrorener FX-Datenqualitätsvertrag festlegen, wie diese Anbieterbalken ohne erfundene Kurswerte behandelt werden. Außerdem ist die historische Verwendung der heute verifizierten Identity-Registry ausschließlich für Dependency-Clusterung explizit zu klären. Danach ist nur ein erneuter kleiner Integritätspilot zulässig; der große Scan benötigt selbst bei späterem READY einen neuen ausdrücklichen Auftrag.
- Der Contract-/Pilot-Code ist lokal mit 41 gezielten und 862 vollständigen Tests, Kompilierung, Repository-Sicherheitscheck, Offline-Smoke, separatem Streamlit-Start, SQLite-Integrität/Checkpoint, idempotentem Resume und Diff-Prüfung abgenommen. Dieser technische Erfolg ändert den fachlichen `NOT_READY`-Status nicht.
