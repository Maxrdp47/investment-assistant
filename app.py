...
def build_scenarios(
    ...
    bull_p, base_p, bear_p = scenario_probabilities(buy_signal, asset_quality, risk_reward, market_phase)
    first_resistance = resistances[0] if resistances else None
    second_resistance = resistances[1] if len(resistances) > 1 else first_resistance
    first_support = supports[0] if supports else None
    second_support = supports[1] if len(supports) > 1 else first_support
