...
## Multi-Asset Discovery v1 Development – terminaler v5-Stand und Integritätskonsolidierung 2026-09-04

- Der unveränderliche Run `mad1-development-a073df9096023f1da079a494` ist terminal `COMPLETED_WITH_FAILURES`. Alle 60.504 Work-Units sind terminal: 41.725 `COMPLETED`, 14.075 historisch `SKIPPED` und 4.704 `FAILED`; 0 sind offen oder aktiv. Der letzte tatsächliche Work-Unit-Abschluss war am `2026-09-03 01:44:03 CEST`. Der später in den Metadaten gespeicherte Abschlusszeitpunkt bleibt als historischer Wert erhalten, wird aber nicht als tatsächliche Laufzeit ausgegeben.
- Die ursprünglichen v5-Control-, Feature- und Outcome-Stores sowie Run-Manifest, Contract, Frozen-Dataset-Manifest, Identity Registry und FX-v2 bleiben bytegenau unverändert. Das historische Run-Manifest behält seinen ursprünglichen `STARTING`-Snapshot; aktuelle Wahrheit wird ausschließlich durch neue versionierte read-only/append-only Diagnoseartefakte ergänzt. Ein erneuter Scheduler-Aufruf erkennt terminale Runs früh und verändert weder Status noch Zeitstempel oder Events.
- Der Development-Scheduler `InvestmentAssistant-MultiAssetDiscoveryV1-Development` ist nach dem terminalen Lauf deaktiviert. Der getrennte `InvestmentAssistant-FX-PIT-Observer` blieb aktiv/`Ready` und wurde nicht verändert. Ein späterer Lauf verwendet weiterhin Prozess-Lock, serielles SQLite-Schreiben, Duplicate-Schutz und checkpoint-fähiges Resume.
- Die saubere Projektion `equity-etf-historical-pit-2026.09.03-v1` übernimmt aus 3.028.079 Equity-/ETF-Quellbalken 3.025.873 aktive gültige Balken. 2.206 unveränderte Quellbalken aus 551 Assets sind fail-closed `INVALID_SOURCE_BAR`; 191 Assets besitzen dadurch keine nutzbare Development-Abdeckung. Aktive Hüllenverletzungen und nichtpositive OHLC-Werte: jeweils 0. Es gab kein Clipping, keine Imputation, keine Interpolation und keine Ersatzdaten. Der neue Dataset-Fingerprint ist `321531c482d844df4d5513b58646b3f41553f47a7b2276517cb8473886c298d6`.
- Alle 209.492 Fälle mit nicht verfügbarer struktureller R-Metrik wurden kausal klassifiziert: vollständig `NON_POSITIVE_STRUCTURAL_RISK`, 0 technische Klassifikationsfehler. R-unabhängige Outcomes bleiben reprozessierbar; es wurde kein R-Wert erzwungen oder erfunden.
- Alle 4.704 fehlgeschlagenen Work-Units sind vollständig aufgelöst: 4.608 Einheiten aus 192 Assets besitzen im eingefrorenen Development-Zeitraum keine zulässige Abdeckung und werden künftig ohne Retry als `SKIPPED` behandelt; 96 Einheiten aus vier Assets enthalten konkrete nichtpositive OHLC-Quellwerte und bleiben deterministisch `FAILED` ohne Retry. Nur echte transiente Lock-/Verbindungs-/Timeoutfehler dürfen erneut versucht werden. Die historischen v5-Zustände wurden nicht umgeschrieben.
- Feature- und Outcome-Store enthalten jeweils 2.564.825 eindeutige Development-Cases. Der Voll-Audit hat alle 2.564.825 Paare in 513 checkpointbaren Batches dekomprimiert und geprüft: identische Case-ID-Mengen, 0 Duplikate, 0 Payload-/PIT-/Contract-Probleme, `quick_check` und vollständiger `integrity_check` für alle drei Stores `ok`, 0 Foreign-Key-Probleme sowie intakte Append-only-Trigger. Die SHA-256-Werte der drei Original-Stores stimmen vor und nach dem Audit exakt überein. Das obsolete per-Case-Feld `full_development_scan_started=false` bleibt unverändert und wird korrekt nicht als heutiger Laufstatus interpretiert.
- Die einzig methodisch saubere Reprocessing-Empfehlung lautet `NEW_VERSIONED_FULL_DEVELOPMENT_RUN_REQUIRED`: Der bereinigte globale Dataset-Fingerprint ist Bestandteil der Case-Identität, daher würde selektives Ersetzen inkompatible Identitäten und Lineage mischen. Diese Empfehlung startet keinen Lauf. Vor einer neuen versionierten Kampagne ist weiterhin ein konservativer Mehr-Worker-Benchmark vorzuschlagen und einzufrieren.
- Es erfolgte keine Performance-Auswertung und keine Hypothesen-, Regel- oder Strategieauswahl. Validation, Holdout, External, True Forward, Paper, Shadow, Broker und Orders bleiben geschlossen; ein großer Reprocessing-Lauf wurde nicht gestartet.
...
- Das Research-Identity-Gate v3 trennt für alle künftigen Research-Epochen `asset_id`, `listing_id` und nur belastbar bekannte `issuer_id`. Zulässige Issuer-Anker sind LEI, SEC CIK, FIGI-/ISIN-/Börsenreferenz oder eine dokumentierte ADR-Beziehung; Firmenname und Ticker allein reichen nicht. Ticker, Börse/MIC, Währung, Instrumenttyp, Assetklasse, Primärstatus, ADR/ADS/GDR, ISIN, FIGI, LEI, SEC CIK, Anteilsklasse, Handelszeitzone und Gültigkeit bleiben versioniert. OHLCV, Kurs, Volumen, Spread, Handelszeiten, technische Marken, Entry, Stop, Ziel, Liquidität und Gaps dürfen nur aus demselben Listing und derselben Währung stammen. XPeng, SAP/ADR, europäische Mehrfachlistings, unbekannte Zuordnung, Tickerwechsel und Bundle-Mischung sind durch allgemeine Regressionstests abgedeckt; es gibt keine XPeng-Sonderregel.
- Das belastbare Multi-Source-Registry `research-identity-registry-2026.08.30-multisource-v2` enthält weiterhin 2.520 getrennte Listings. Von 2.490 issuerfähigen Listings sind 2.180 über starke, versionierte Anker verifiziert (87,550201 %), 293 bleiben `CANDIDATE_UNVERIFIED`, 17 issuerfähige Fälle ungelöst und 0 im Konflikt; einschließlich nicht issuerfähiger Instrumente sind 340 Dependencies sichtbar unbekannt und tragen 0 zum effective N bei. 2.153 verifizierte Issuer-Cluster und alle vier vorab dokumentierten ADR-/Multi-Listing-Gruppen wurden erkannt. Registry-Fingerprint `d8bd34a3bac724f6ff15f4d33a03efe7b517518b2a71208046be11bc1530387e`, Mapping-Fingerprint `14091b91d4c9c7b33b9589d411402cb7ffdab4cf5417d42eed3342fd4ed73cc8`; das outcome-unabhängig festgelegte 80-%-Coverage-Gate besteht. Der weiterhin reale SEC-403 wird durch einen gepinnten SEC-CIK-Mirror, einen eingefrorenen OpenFIGI-Listing-Snapshot und eng begrenzte offizielle Issuer-/ADR-Beziehungen transparent ergänzt; Namen oder Ticker allein erzeugen keine Verifikation.
- Der Fibonacci-READY-Auftrag `d941320c-48ca-43fb-9bd9-0c362d8873ea` ist ohne Doppelrun `COMPLETED`. Sein Vertrag war fachlich identisch zum immutable Broad-v1-Pullback-Audit mit 0,618–0,786, zwei gleich breiten Nachbarzonen und kontinuierlicher Pullbacktiefe. Resultat `4a3f776a-17a5-45f6-b0b7-0b9fa51272bb` übernimmt die vorhandene B-/Development-Evidenz: 23.575 Treatment-Fälle, +0,0446 R, Profitfaktor 1,072, nur 5/9 positive Jahre und unter Zusatzslippage negativ. Keine Validation, kein Holdout, keine Extension und keine Strategieaktivierung.
- Der FX-Carry-Auftrag `1f6201fe-2cbd-4fc1-9bf1-291a4221bdfa` ist mit Resultat `a4c0b1c7-ab1d-4802-bfc6-48d869843cb6` als transparente Teilpipeline `COMPLETED`. EUR/USD, USD/JPY und GBP/USD besitzen versionierte Basis-/Gegenwährungs-, Inversions-, Vorzeichen-, Session-, Tagesabschluss-, `known_at`-/`available_at`- und Revisionsverträge. Das historische Resultat selbst bleibt unverändert `inconclusive` mit Stichprobe 0; spätere Infrastruktur-Evidenz wurde nur append-only referenziert.
- Die immutable Erweiterung `fx-historical-pit-2026.08.29-v1` hält weiterhin byte-identisch 12.779 tägliche OHLC-Kursbeobachtungen. Die getrennte aktive Projektion `fx-historical-pit-2026.09.01-v2` behebt ausschließlich die Datenzulassung: Der v1-Validator prüfte die obere Hüllengrenze, ließ aber 231 ausschließlich unterseitige Verstöße (`Open`/`Close < Low`) passieren. Alle 231 sind mit vollständigen OHLC-, Größen-, Quellen-, Session-, Inversions- und Importfeldern als `INVALID_SOURCE_BAR` archiviert und ohne Ersatz aus der aktiven Evidenz ausgeschlossen. Die ursprüngliche Provider-/Session-Ursache ist aus dem konservierten v1-Material nicht belastbar beweisbar und bleibt entsprechend `UNKNOWN`; es gab weder Clipping, Imputation noch Interpolation. Die 229 schon beim v1-Import verworfenen Balken bleiben eine getrennte Lineage-Gruppe. v2 enthält 12.548 aktive gültige Bars und exakt 0 aktive Hüllenverletzungen; Dataset-Fingerprint `a3c41cddd06d7b24596bac5f1e375868a86784ea5a6feeacd9b44f49598c5c91`.
- Der reine Datensammler `fx-pit-observer-2026.08.29-v1` besitzt eine eigene append-only Datenbank `runtime/fx_forward_pit.sqlite3`, Process Lock, Idempotenz, Revisionen, Missingness, Source Health, Provenienz und 28 Sperrtrigger. Offline- und Live-Pilot sind abgeschlossen. Zwei echte geplante Läufe starteten am 2026-08-29 und 2026-08-30 jeweils um 21:45 Uhr und endeten fehlerfrei. Der read-only Scheduler-Audit `fx-pit-scheduler-audit-2026.08.31-v3` weist genau einen kanonischen Windows-Task `InvestmentAssistant-FX-PIT-Observer` unter Benutzer `maxwi`, Working Directory `C:\investment-assistent`, `StartWhenAvailable=true`, `WakeToRun=false`, drei Retries und `IgnoreNew` nach. Der nächste Lauf ist für 2026-08-31 21:45 Uhr geplant. Fehlende Policy-Rate-, Erwartungs-/Vintage- und Bid/Ask-Adapter bleiben ehrlich `NO_RELIABLE_DATA`; beide geplanten Läufe erzeugten keinerlei Strategie-, Trade-, Paper-, Shadow- oder Broker-Ausgabe.
- Der FX-PIT-Observer ist ausschließlich Datensammlung: `strategy_signal_generated=false`, `trade_decision_generated=false`, `paper_trade_generated=false`, `shadow_order_generated=false` und `broker_order_allowed=false`. Er reaktiviert weder Legacy Forward v1 noch einen FX-Strategy-Forward. Der bestehende KB-Work-Request, das Experiment und das alte Resultat blieben unverändert abgeschlossen; Identity-, Historical-FX- und Collector-Pilot-Artefakte wurden als drei neue append-only Referenzen ergänzt.
- Abnahme dieser Erweiterung: 45 gezielte Identity-/FX-/Collector-/KB-Tests und 834/834 vollständige Regressionstests bestanden. Vollständige Python-Kompilierung, Repository-Sicherheitscheck, Offline-Smoke inklusive separatem Streamlit-Start, Swing-Preflight, Collector-Audit und `git diff --check` sind erfolgreich. Knowledge Base und Collector-Datenbank melden Integrität `ok`; die KB besitzt 0 Foreign-Key-Probleme und weiterhin genau ein kanonisches FX-Resultat. Der normale Smoke-Test mit Live-Daten war im eingeschränkten Testprozess wegen nicht verfügbarer Yahoo-Daten nicht vollständig; der ausdrücklich verlangte Offline-Smoke war grün.
- Failed Seller Attempts bleibt als unveränderte kausale Development-Feature-Epoch mit Work Request `d4dbaf10-b321-4223-9a0b-91f0bc245151`, Resultat `07f46840-35e1-41f4-949c-264c7cf08fbe` und Feature-Contract-Fingerprint `2768a54a4ee619d4c2c66982cd6f80cf5f950f1c5a8f2c19f45cff53fb15a400` abgeschlossen. Die neue read-only Dependency-Neueinordnung `failed-seller-dependency-reclassification-2026.08.30-v2` verändert keine der 379.039 Cases oder Rohmetriken: 338.229 Beobachtungen besitzen verifizierte Dependencies (89,233298 %), 40.810 bleiben unbekannt, 1.923 Issuer-Cluster sind vertreten und das konservative maximale nicht überlappende 42-Kalendertage-Effective-N beträgt 69.534. Für die vier unveränderten Varianten beträgt das effective N 13.203 / 1.133 / 43.580 / 34.919. Die positive Development-Deskription ist damit clusterinterpretierbar, bleibt aber verbindlich `INCONCLUSIVE_RETAINED`; keine Hochstufung, kein neuer Attempt, keine Validation, kein Holdout, kein Filter und keine Strategie. Die Knowledge Base referenziert das versionierte Assessment append-only, während der Kern des Originalresultats byte-logisch unverändert blieb.
- Der finale Multi-Asset-Precheck `multi-asset-final-precheck-2026.08.31-v1` besteht alle fünf Gruppen `A_RESEARCH_IDENTITY`, `B_DEPENDENCY_EFFECTIVE_N`, `C_RESEARCH_INTEGRITY`, `D_FX_OBSERVER` und `E_SAFETY`. Endstatus ist `READY_TO_DEFINE_MULTI_ASSET_DISCOVERY_V1_CONTRACT`. Das erlaubt ausschließlich, im Planungs-Chat den deterministischen Discovery-v1-Vertrag zu definieren und danach einzufrieren; der große Multi-Asset-Scan selbst bleibt `NOT_STARTED`, ebenso neue Labels, Validation, Holdout, Forward, Paper, Shadow, Broker und Produktion.
- Der deterministische Vertrag `multi-asset-opportunity-discovery-2026.08.31-v1` ist jetzt vollständig definiert und eingefroren. Contract-Fingerprint: `68994e462f90e2a4f1ad4adbdc858cc43fb0ddb85b0c2662d0f647e1dfa6c05a`; authoritative Implementierungs-Freeze `implementation-r4`: `0d5a0cb5634398ccdc8e9fe55a702ea1b4c713cbf85415bf710d6d503d333aef`; Code-Fingerprint: `c76268ca3d274324996bc9bf9e68a26b2bc257edf384aa39b8820f26e60154ac`. Eingefroren sind getrennte Daily-Analysen für Aktien, ETFs, FX und Krypto, Next-Open, 252 Beobachtungen, harte Stage-Zensierung, exakt drei Safe- und drei beobachtende Sell-Zonen, MFE/MAE/R, getrennte Deterioration, explizite Missingness, fail-closed Dependencies sowie physisch getrennte append-only Feature-/Outcome-Stores. Es gibt keinen prädiktiven Vorfilter, keinen Opportunity-Gesamtscore und keine automatische Regelableitung.
- Die neue outcome-unabhängige Policy `multi-asset-historical-dependency-policy-2026.09.01-v1` trennt Mapping-Verfügbarkeit strikt von historischer Beziehungsgültigkeit. Heutige CIK-/FIGI-/ADR-Identität wird nicht rückwirkend verwendet; `valid_from`/`valid_to`, belastbare zeitliche Evidenz und Corporate-Action-Grenzen sind Pflicht. Die aktuelle Registry besitzt 0 solche historischen Beziehungsdatensätze, daher bleiben alle 11 Pilotfälle `DEPENDENCY_UNKNOWN` und tragen exakt 0 zum issuer-adjustierten Effective N bei. Das ist fail-closed und verändert weder Feature, Candidate, Entry, Safe Zone noch Outcome. Policy-Fingerprint: `706b0b9e438464405f18d0972d49d34c553538c1236c3e7adb8fe37157214393`.
- Der unveränderte technische Pilot über dieselben elf festen Fälle wurde mit FX-v2 erneut ground-up ausgeführt. Alle 20/20 Gates bestehen, insbesondere `no_ohlc_envelope_anomalies`, PIT/Next-Open, Stage-Censoring, Safe-/Sell-Zones, Ratchet, Feature-/Outcome-Trennung, Determinismus, Missingness und Dependency-Fail-Closed. Case-Digest `b9aa3f5c4cf0097def4d4480e812d1bbec23f764543d36b59b91cb270014b859`, authoritative Pilot-Fingerprint `65e3243726be47ee0c9a61bd5e8e35512dbe1a8453cecd1776cbbe0be2c3a6fe`, Implementierungs-Freeze `a93f68ee3eec1fddb37ea7a19d5616cccba7d43ece6146b01f8789a9d98b4a8e`. Der Contract-Fingerprint blieb unverändert.
- Der finale Gate `multi-asset-final-development-readiness-2026.09.01-v1` besteht 7/8 Gruppen. `A_RESEARCH_IDENTITY`, `B_DEPENDENCY_EFFECTIVE_N`, `C_RESEARCH_INTEGRITY`, `D_FX_HISTORICAL_DATA_QUALITY`, `E_FX_OBSERVER`, `F_PIT_LEAKAGE_AND_PILOT` und `G_SAFETY` sind `PASS`. `H_DEVELOPMENT_EXECUTION_CONTRACT` ist hart `FAIL`, weil der immutable Vertrag `research_role=technical_integrity_pilot_only`, `candidate_generation.mode=fixed_representatives_for_integrity_pilot`, `full_development_scan_allowed=false`, `pilot_contract.large_scan_allowed=false` und nur Pilot-Stores definiert. Diese Felder zu ändern oder zu übersteuern würde die eingefrorene Contract-Semantik verändern. Verbindlicher Status bleibt daher `NOT_READY_TO_START_MULTI_ASSET_DISCOVERY_V1_DEVELOPMENT`; Gate-Fingerprint `add32aced165a12a8ee76dcee54fd14ac81b222f1dae3ba8854ded4cd1f9385d`.
- Produktionswirkung dieses Auftrags: keine. Kein Development-Runner und keine zusätzliche Windows-Aufgabe wurden erstellt; Validation, Holdout, External, True Forward, Paper, Shadow, Broker und automatische Orders blieben geschlossen. Frozen Dataset, Broad-v1, Buyer, Failed-Seller, FX Forward Observer, alte Identity-Registry, FX-v1 sowie alte Pilot-/Freeze-Artefakte blieben hash-identisch unverändert. Vollständiger Nachweis: `research_reports/MULTI_ASSET_DISCOVERY_V1_FINAL_INTEGRITY_REPAIR_2026-09-01.md`.
- Lokale Abnahme dieses Reparaturstands: 47 gezielte Tests und 878/878 vollständige Regressionstests bestanden; die finale reparaturspezifische 46-Test-Teilmenge wurde zusätzlich erneut grün ausgeführt. `python -m compileall .`, Repository-Sicherheitscheck, Offline-Smoke, separater lokaler Streamlit-Start mit HTTP 200, SQLite-Quick-Checks, deterministischer Pilot-Replay, append-only Resume und `git diff --check` sind erfolgreich. Der Code-Commit ist `c4435929046eae01bd561c51bcef3ffc237dd208`; der Push wurde von der Ausführungs-Sicherheitsfreigabe blockiert, daher ist für diesen Commit noch kein GitHub-Actions-Ergebnis behauptet.
- Abschlussabnahme dieses Gates: 29/29 gezielte Identity-/Dependency-/Failed-Seller-/FX-/KB-/Precheck-Tests und 850/850 vollständige Regressionstests bestanden. `python -m compileall .`, Repository-Sicherheitscheck, Offline-Smoke, integrierter und separater lokaler Streamlit-Start sowie `git diff --check` sind erfolgreich. Die Knowledge Base meldet `integrity_check=ok`, 0 Foreign-Key-Probleme und exakt eine append-only Failed-Seller-Assessment-Referenz.
- Die Research-KB ist nach diesen drei Abschlüssen integer (`integrity_check=ok`, 0 Foreign-Key-Probleme), jeder Request besitzt genau ein direkt verknüpftes persistentes Resultat und Artefaktlinks. Gold/Silber `4fdfb983-ddbc-4178-bd36-7aa34267df0b` sowie Wasser `3721453e-158f-42cb-8d76-a28f054b7d97` bleiben unverändert `READY`.
...
Von den fünf KB-Aufträgen aus Roadmap G2.12 sind Fibonacci, FX-Carry-PIT und Failed Seller methodisch abgeschlossen. Gold/Silber und Wasser bleiben optionale, unangetastete `READY`-Aufträge; keiner dieser Bausteine ist automatisch eine Strategie.
...
- Verkäufer-Pushs/`failed_seller_attempts` und `close_location = (close-low)/(high-low)` besitzen jetzt einen getrennten, eingefrorenen Development-Featurevertrag. Entsprechend dem exakten KB-Vertrag wurden nur Attempt Count genau 1/2 und Close Location ≥0,70/≥0,80 einzeln geprüft; keine Kombination, freie Schwelle, Validation oder Strategieänderung. Broad-v1, Long-v1 und aktive Signale blieben unverändert.
- FX-/Carry-Research besitzt jetzt eine versionierte, append-only Point-in-Time-Grundpipeline für Paaridentität, Inversion, Zeitpunkte und Revisionen. Historische Erwartungen, Surprises und Vintages bleiben mangels belastbarer lokaler Daten ausdrücklich `UNAVAILABLE`; COT/Kosten sind nur Kontext. Es existiert weiterhin keine FX-Richtungsregel oder Strategie.
