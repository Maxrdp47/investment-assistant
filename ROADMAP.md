...
## Multi-Asset Discovery v1 Development – v5 terminal, Reprocessing-Review offen 2026-09-04

- Der kanonische Run `mad1-development-a073df9096023f1da079a494` ist unveränderlich terminal `COMPLETED_WITH_FAILURES`. Alle 60.504 Work-Units sind abgeschlossen: 41.725 `COMPLETED`, 14.075 historisch `SKIPPED`, 4.704 `FAILED`, 0 offen/aktiv. Letzter tatsächlicher Work-Unit-Abschluss: `2026-09-03 01:44:03 CEST`.
- v5-Control-, Feature-/Outcome-Stores, Run-Manifest, Contract, Frozen Dataset, Identity Registry und FX-v2 bleiben historische read-only Referenz. Insbesondere wird weder der terminale Status nachträglich zu `COMPLETED` umgeschrieben noch der historische `STARTING`-Snapshot im Manifest korrigiert. Neue Wahrheit liegt ausschließlich in versionierten Diagnose-/Readiness-Artefakten.
- Der v5-Scheduler ist deaktiviert; der getrennte FX-PIT-Observer bleibt `Ready`. Terminale Scheduler-Aufrufe enden künftig vor schweren Audits und ohne neue Events oder Zeitstempel. Für neue Runs werden fehlende Development-Abdeckung und deterministische Datenfehler nicht mehr sinnlos wiederholt; nur transiente Lock-/Verbindungs-/Timeoutfehler sind retryfähig.
- Die neue getrennte Equity-/ETF-Projektion schließt 2.206 unzulässige Quellbalken aus 551 Assets ohne Datenkosmetik aus und enthält 3.025.873 aktive gültige von 3.028.079 Rohbalken. 191 Assets besitzen keine nutzbare Development-Abdeckung. Aktive OHLC-Hüllenverletzungen und nichtpositive Werte sind jeweils 0; der neue Dataset-Fingerprint lautet `321531c482d844df4d5513b58646b3f41553f47a7b2276517cb8473886c298d6`.
- 209.492 strukturelle R-NA-Fälle sind vollständig als `NON_POSITIVE_STRUCTURAL_RISK` klassifiziert; 0 technische Klassifikationsfehler. R-unabhängige Outcome-Felder bleiben reprozessierbar, ohne einen künstlichen R-Wert zu erzeugen.
- Von 4.704 v5-Fehleinheiten sind 4.608 fehlende historische Development-Abdeckung und 96 deterministische nichtpositive OHLC-Quellfehler; 0 bleiben ungeklärt. Der Voll-Audit hat alle 2.564.825 Feature-/Outcome-Paare in 513 checkpointbaren Batches dekomprimiert und geprüft: identische Case-ID-Mengen, 0 Duplikate, 0 Payload-/PIT-/Contract-Probleme, alle SQLite-Integritätsprüfungen `ok`, 0 Foreign-Key-Probleme und unveränderte SHA-256-Werte der Original-Stores. Das obsolete per-Case-Feld `full_development_scan_started=false` bleibt als historische Metadaten erhalten und wird nicht als Laufstatus verwendet.
- Der Integritätsstand ist damit bereit für den technischen Reprocessing-Review. Die einzige zulässige Empfehlung lautet `NEW_VERSIONED_FULL_DEVELOPMENT_RUN_REQUIRED`, weil der globale Dataset-Fingerprint Bestandteil jeder Case-Identität ist und eine selektive Reparatur inkompatible Lineage mischen würde. Diese Empfehlung ist keine Startfreigabe. Ein neuer Lauf benötigt zuerst Review, neue Version, unveränderte Gate-Reihenfolge und einen vorab festgelegten Mehr-Worker-Benchmark.
- Bis dahin und auch durch diese Reparatur gilt: keine Performance-Auswertung, keine Hypothesen-/Regelwahl, kein großer Reprocessing-Lauf und keine Öffnung von Validation, Holdout, External, Forward, Paper, Shadow, Broker oder Orders.
...
- Provenienzbereinigung und isolierte Ground-up-Reproduktion sind abgeschlossen: Originalcode auf Commit `a2bc13b` gefunden, 2.520/2.520 Assets und sämtliche logischen Records, Fingerprints, Metriken, Gates sowie die Entscheidung exakt reproduziert. Result Integrity und Software Provenance sind `VERIFIED`; das vollständige Erratum steht in `research_reports/BUYER_CONFIRMATION_VALIDATION_PROVENANCE_ERRATA_2026-08-28.md`.
- Die Research-KB hält das negative Resultat und eine idempotente negative Validation-Bewertung append-only fest. Die breitere Quellhypothese bleibt methodisch getrennt `RAW/B`; der verworfene Challenger wird von Reporting und Planung nicht als aktiver C-, Holdout-, External- oder Produktionskandidat angeboten.
...
## Aktuelles Gate vor der nächsten Research-Epoch – 2026-08-31
...
- Der allgemeine Legacy-/Equity-Observer bleibt deaktiviert, weil dort Datenbeobachtung noch nicht sicher von Strategie-, Signal-, Paper- und Shadow-Pfaden getrennt ist. Der neue eigenständige `FX_PIT_OBSERVER` ist dagegen technisch getrennt, append-only und darf ausschließlich Daten sammeln; sämtliche Strategie-, Trade-, Paper-, Shadow- und Broker-Ausgänge sind fest gesperrt.
- Der nächste große Schritt heißt `Multi-Asset Opportunity Discovery v1`. Der Scan bleibt ausdrücklich `NOT_STARTED`: keine neue Candidate Generation, keine neuen Multi-Asset-Labels, kein Challenger und keine ungesehene Stufe. Der finale Infrastruktur-/Methodik-Precheck ist jetzt `READY_TO_DEFINE_MULTI_ASSET_DISCOVERY_V1_CONTRACT`: 2.180 von 2.490 issuerfähigen Listings sind verifiziert (87,550201 %), alle bekannten ADR-/Multi-Listing-Gruppen sind aufgelöst, 340 unbekannte Dependencies bleiben fail-closed und zählen nicht als unabhängig, der Failed-Seller-Effective-N ist korrigiert, genau ein FX-Scheduler besitzt zwei echte geplante Erfolgsruns und sämtliche Safety-Gates bestehen. READY erlaubt nur die anschließende deterministische Vertragsdefinition und den Freeze im Planungs-Chat; es startet keinen Scan.
- Der Discovery-v1-Vertrag bleibt mit Contract-Fingerprint `68994e462f90e2a4f1ad4adbdc858cc43fb0ddb85b0c2662d0f647e1dfa6c05a` unverändert eingefroren. Der FX-Integritätsblocker wurde ohne Datenkosmetik geschlossen: Die getrennte Projektion `fx-historical-pit-2026.09.01-v2` archiviert alle 231 unterseitigen Hüllenverstöße als `INVALID_SOURCE_BAR`, verwendet 12.548 aktive gültige Bars und weist exakt 0 aktive Hüllenverletzungen aus. Kein Clipping, keine Imputation, kein Ersatz durch heutige Daten; FX-v1 bleibt immutable.
- Die historische Dependency-Semantik ist mit `multi-asset-historical-dependency-policy-2026.09.01-v1` deterministisch fail-closed. Heutige Identität ist kein historisches Trading-Feature und ohne belastbares zeitliches Gültigkeitsfenster keine historische Dependency. Die aktuelle Registry enthält 0 solche Zeitbelege; die 11 Pilotfälle bleiben daher unbekannt und zählen 0 zum Effective N. Valid-from/valid-to, Fusionen, Spin-offs, ADR-/Primärlisting, Share Classes, Ticker-/Listingwechsel und unbekannte Beziehungen sind getestet.
- Der erneute elf Fälle umfassende Integritätspilot besteht mit FX-v2 alle 20/20 Gates. `no_ohlc_envelope_anomalies`, PIT, Leakage, Next-Open, Stage-Censoring, Safe-/Sell-Zones, Ratchet, Store-Trennung, Determinismus und Dependency-Fail-Closed sind `PASS`; Pilot-Fingerprint `65e3243726be47ee0c9a61bd5e8e35512dbe1a8453cecd1776cbbe0be2c3a6fe`.
- Der separate finale Development-Readiness-Gate besteht 7/8 Gruppen. Der einzige harte Fehler liegt jetzt im unveränderten Vertrag selbst: Er ist ausdrücklich `technical_integrity_pilot_only`, verwendet feste Pilot-Repräsentanten, verbietet den Full Scan und definiert keine Development-Stores. Eine automatische Ausführung würde damit genau den eingefrorenen Contract übersteuern. Verbindlicher Status bleibt `NOT_READY_TO_START_MULTI_ASSET_DISCOVERY_V1_DEVELOPMENT`; kein Runner, kein Scheduler und kein Development-Run wurden gestartet.
- Ein späterer Full-Development-Start erfordert zuerst einen ausdrücklich autorisierten, vor Ergebnissichtung eingefrorenen Development-Ausführungsvertrag beziehungsweise eine neue Contract-Version mit Full-Universe-Modus, Development-Stores, Checkpoint-/Resume- und Runner-Semantik. Diese Entscheidung darf nicht als technische Reparatur in v1 hineingeschrieben werden. Validation und Holdout bleiben geschlossen.
- Der Reparatur-/Gate-Code ist lokal mit 47 gezielten und 878 vollständigen Tests, einer erneut grünen reparaturspezifischen 46-Test-Teilmenge, Kompilierung, Repository-Sicherheitscheck, Offline-Smoke, separatem lokalem Streamlit-Start mit HTTP 200, SQLite-Integrität, deterministischem Replay, idempotentem Resume und Diff-Prüfung abgenommen. Der Push von Commit `c443592` wurde durch die Ausführungs-Sicherheitsfreigabe blockiert; ein CI-Erfolg wird daher erst nach tatsächlich genehmigtem Push dokumentiert.
...
- Der belegte Scheduler-Stand umfasst zwei geplante FX-Observer-Läufe am 29. und 30. August jeweils um 21:45 Uhr, beide erfolgreich und ohne Strategie-/Tradeausgabe. Lokale Abschlussabnahme: 29/29 gezielte Tests und 850/850 vollständige Regressionstests, Kompilierung, Repository-Sicherheitscheck, Offline-Smoke, integrierter und separater Streamlit-Start, KB-Integrität sowie Diff-Prüfung sind grün. GitHub wird erst nach dem finalen Push als erfolgreich dokumentiert.
...
Status 2026-08-23: Die automatische append-only Verknüpfung mit neuen echten Forward-Signalen ist aktiv. Der Hintergrundrunner aktualisiert die offizielle CFTC-Quelle nach der unveränderbaren Signalablage fail-open; ein erstmals dabei beobachteter Report ist für dieses frühere Signal noch nicht zulässig, steht aber mit seinem tatsächlichen lokalen `first_seen_at` für spätere Signale bereit. Je Signal werden Asset-/Listingbezug, explizites CFTC-Mapping, Reporttyp/-stichtag, belegte Verfügbarkeit, Teilnehmerklassen, Net/OI, 1W/4W, 52-Wochen-Perzentil/Z-Score, Extrem/Umkehr, Divergenzen und Fingerprints in einem getrennten Sidecar gespeichert. Ohne kausal zulässigen Stand bleibt `cot_context_unavailable`; ein späterer Report wird niemals eingesetzt.

Der aktuelle geprüfte Speicher umfasst 61.944 CFTC-Reportversionen und 1.085 beim offiziellen Abruf am 2026-08-23 erstmals lokal belegte Verfügbarkeiten. Für alle 29 bestehenden Forward-Signale existiert ein Sidecar: 3 Signale (`CRVL`, `COP`, `CVX`) besitzen einen zulässigen breiten US-Aktienmarktkontext aus `tff_futures_only` mit Berichtsstichtag 2026-08-11; 26 bleiben unavailable. Der Forward-Snapshot blieb byteidentisch. Historische Veröffentlichungskalender, emittentenspezifische Positionierung und gereifte Forward-Vergleiche fehlen weiterhin. Der gesamte Layer ist strikt `shadow_only / research_only`; Signale, Scores, Gewichte, Risiko, Broad-Feature-Freeze und Long-v1 bleiben unverändert.
...
- **FX Carry Point-in-Time-Pipeline – Infrastruktur erweitert, Coverage teilweise**
   Work Request `1f6201fe-2cbd-4fc1-9bf1-291a4221bdfa`, Experiment `793d4731-4de5-4c16-a7f3-dd44dea1761c`, Resultat `a4c0b1c7-ab1d-4802-bfc6-48d869843cb6`: unverändert `COMPLETED` und `inconclusive`; das alte Resultat wurde nicht umgeschrieben. EUR/USD, USD/JPY und GBP/USD besitzen reproduzierbare Paar-, Inversions-, Vorzeichen-, Session-, Verfügbarkeits- und Revisionsverträge. Neu vorhanden sind 12.779 tägliche Kursbeobachtungen 2010-01-01 bis 2026-08-28 mit konservativem Availability-Cutoff sowie ein getrennter täglicher append-only `FX_PIT_OBSERVER` um 21:45 Uhr. Offline-/Live-Pilot und Safety-Audit sind bestanden; Kurs- und vorhandene offizielle COT-Beobachtungen werden gesammelt, verpasste Läufe nachgeholt. Historische Policy Rates, Erwartungen, Konsense, Vintages, Interventionen, implied Volatility und Bid/Ask bleiben `UNAVAILABLE`; keine FX-Auswertung, Richtung, Strategie, Paper-/Shadow-Order oder Brokerfunktion.
- **Failed Seller Attempts – Development-Feature-Epoch plus Dependency-Neueinordnung abgeschlossen**
   Work Request `d4dbaf10-b321-4223-9a0b-91f0bc245151`, Experiment `f0ae30fb-d00e-461f-84ad-ac3110f4ae49`, Resultat `07f46840-35e1-41f4-949c-264c7cf08fbe`: unverändert `COMPLETED`. Das versionierte read-only Assessment über dieselben 379.039 Cases weist 89,233298 % verifizierte Beobachtungs-Dependencies, 40.810 unbekannte Beobachtungen, 1.923 Issuer-Cluster und ein konservatives effective N von 69.534 aus. Die vier Rohmetriken und der einzige präregistrierte Versuch je Variante bleiben unverändert; die Einstufung bleibt `INCONCLUSIVE_RETAINED`. Das Assessment ist append-only mit dem Originalresultat verknüpft; keine Kombination, Validation, Holdout, Filter- oder Strategieaktivierung.
...
Status 2026-08-23: Strategie, unabhängige gemeinsame Risk Engine, Ausführungssimulation, Positionszustand und Append-only Audit sind technisch getrennt. Zusätzlich besitzt jeder Shadow-Orderentwurf einen append-only Execution-Beobachtungsvertrag, der tatsächliche Marktdaten, theoretischen Systementwurf, spätere Shadow-Auswertung und simulierte Kosten strikt trennt. Reale Quotes erfordern exaktes Listing, Quelle und Quellzeitpunkt; veraltete Quotes bleiben als `stale` sichtbar. Ohne belastbare Quelle werden Bid, Ask, Mid, Spread, Slippage, Fill, Teilfill, Orderbuch und Brokerablehnung nicht geschätzt, sondern als `real_execution_quote_data_unavailable` gespeichert. Scanner, Paper-Bot und Shadow-Live verwenden denselben versionierten Risiko- und Orderplan; die Risk Engine kann von keinem Research-Challenger umgangen werden.

Aktueller Bestand: 3 Shadow-Orderentwürfe, 3 explizite Missingness-Beobachtungen, 0 echte Bid-/Ask-Quotes, 0 echte Trade-only-Beobachtungen und keine Fill-/Slippage-Evidenz. Im Projekt ist keine belastbare kostenlose automatische Read-only-Quotequelle für die konkreten Listings vorhanden; nur die geprüfte Adapter-Schnittstelle ist vorbereitet. Es existiert weder Broker-Adapter noch Echtgeldpfad, und Providerfehler dürfen Scan, Forward-Signal, Paper-Zyklus oder Broad Research nicht blockieren.
