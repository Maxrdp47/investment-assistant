...
| `swing_shadow_live.py` | Getrennte append-only Shadow-Orderentwürfe aus aktuellen Signalen ohne Übertragung. Echte Mikrostruktur-/Ausführungsdaten werden nur mit belastbarer Quelle gespeichert und niemals geschätzt. |
...
| `cot_positioning.py` | Strikt beobachtender COT-Shadow-Layer mit offiziellem CFTC-Abruf, Point-in-Time-Sperre, originalen Teilnehmerklassen, Merkmalen, Marktzuordnung, append-only Speicher und getrenntem Strategie-Vergleich ohne Produktionswirkung. |
...
| `tests/test_swing_background_runner.py` | Drei Tests für exakt einmalige regionale 2.520-Asset-Abdeckung, objektive Null-Trade-Speicherung und Schutz bei vollständigem Provider-Ausfall. |
...
- Die laufende historische Kampagne steht am zuletzt geprüften 2026-08-22 um 19:55 Uhr bei 243/248 Jobs beziehungsweise 97,98 %. Runde A und B sind mit jeweils 80/80 abgeschlossen, Runde C steht bei 75/80. Der zuletzt abgeschlossene C-Job verarbeitete alle 315 Assets ohne Ladefehler. Der breite Runner bleibt bei fünf offenen Jobs gesperrt und verändert weder Queue noch Fälle, Dataset oder Kampagnendatenbank.
...
- Der automatische Folgepfad ist vorbereitet, aber noch nicht ausgelöst: Der unveränderte Kampagnen-Wrapper ruft nach einem erfolgreichen Kampagnenprozess den breiten Runner auf. Dieser startet erst nach exakt 248/248, vollständigem Walk-Forward-Audit, identischem finalisierten Dataset-Fingerprint und append-only Übergangsnachweis. Je Aufruf werden höchstens 16 Assets verarbeitet, sodass Schutzfenster, Produktionslocks und normale Scans zwischen den Blöcken erneut Vorrang erhalten. Es werden keine neuen Marktdaten geladen.
...
- Der autonome Paper-Bot ist in den bedienungsfreien Swing-Hintergrundlauf eingebunden. Er verarbeitet Marktdaten, Signal, Risk Check, virtuelle Order, kausalen Fill/kein Fill, Position, Stop, Ziele, Teilverkauf, Exit und Auswertung ohne Nutzereingriff; Läufe und Ereignisse sind append-only, idempotent, restartfähig und bei fehlenden Daten fail-closed. Der geprüfte lokale Stand umfasst 16 Zyklen, drei Signale, zwölf Signalereignisse und 32 Zyklusereignisse; diese kleine aktuelle Stichprobe ist nicht freigabefähig.
- Die Shadow-Live-Grundlage erzeugt aus aktuellen Signalen denselben Orderentwurf, der später einem Live-Pfad zugrunde liegen könnte, überträgt ihn aber niemals. Drei getrennte Entwürfe sind gespeichert; es liegen noch keine belastbaren echten Bid-/Ask-/Ausführungsbeobachtungen vor. Fehlende Spreads, Slippage oder Fill-Qualität bleiben deshalb unbekannt und werden nicht geschätzt.
...
| Forward-Sammlung | für künftige neue Hintergrundsignale aktiv; Yahoo-News nur mit tatsächlichem `first_seen_at` |
...
Im Arbeitsverzeichnis liegt der abgeschlossene, aber noch nicht committete UI-, Empfehlungs-, Performance-, Prognose- und Swing-Scanner-Ausbau. Er umfasst das 1.726-Asset-Prognosewochenuniversum, den entkoppelten Horizontkalender, die automatische Auswertung sowie den 2.520-Asset-Swing-Finder ohne Tiefenanalyse-Cutoff, mit Assetklassen-Funnel, regionalem Echtbetrieb, Strategie-Freezes, technischen Research-Challengern, gemeinsamer Risk Engine, autonomem Paper-Bot, brokerlosem Shadow-Live, Stopout-Forensik, gesichertem automatischem Broad-Research-Handoff und einem rein vorbereitenden Short-Readiness-Layer im noch nicht gestarteten Broad-Vollpass. Die vorhandene Long-v1-Baseline, ihre Einzel-Scores und Gewichte wurden nicht fachlich umgewichtet oder durch einen Challenger ersetzt; aktuelle Kandidaten, Scanner, Risk Engine, Paper und Shadow bleiben ausschließlich Long.

Verbindliche Arbeitsreihenfolge seit 2026-08-11:

1. **Swing Trade Finder zuerst.** Echte EWL-/LT.NS-Forward-Trades weiter sammeln und auswerten; keine neuen Setup-Arten vor belastbarer Reifung.
2. **Effiziente Prognosedaten danach.** Entkoppelte Horizonte, Fälligkeitsauswertung, Datenqualität und Wochenbetrieb weiter real beobachten.
3. **Andere Produktarbeit später.** Nur kritische Fehler bei Stabilität, Datenschutz, Datenintegrität oder falschen Ergebnissen dürfen die ersten beiden Blöcke vorübergehend unterbrechen.

Prioritätsblock 1 – nächste Validierungs- und Ausbauschritte:

- Die historische Challenger-Kampagne kontrolliert bis 248/248 fortsetzen. Der zuletzt geprüfte relevante Stand 243/248 erlaubt weder einen Broad-Start noch eine Produktionsbehauptung; die automatische Weitergabe übernimmt nach Abschluss die erneute Vollprüfung.
- Nach dem gesicherten Übergang den breiten Frozen-Research-Pass über alle 2.520 Asseteinträge in kleinen Resume-Blöcken abschließen. Sein technischer Speicher ist intakt, enthält aber bewusst noch 0 Assetabschlüsse und 0 Research-Kandidaten. Erst dann werden Feature-/COT-Abdeckung, Development-Basis, A/B/C-Hinweise und feste Parameter-Nachbarschaften real berechnet. Validation/Holdout bleiben bis zu einem manuell bestätigten C-Freeze gesperrt.
- Autonomen Paper-Bot und Shadow-Live über genügend aktuelle Signale, Markt-/Volatilitätsregime und Betriebszustände laufen lassen. Der Paper-Bestand ist noch klein; echte Shadow-Mikrostrukturbeobachtungen fehlen. Keine Strategieänderung aus unreifer Evidenz.
...
- Die echten EWL-/LT.NS-Signale und alle folgenden Long-v1-Fälle append-only bis Einstieg, Verpassung, Ungültigkeit, Stop/Zielen oder Ablauf verfolgen und segmentiert auswerten.
- Die neutralisierte v3-Pipeline über weitere reale Scans beobachten und echte ETF-/Aktien-Forward-Ergebnisse sammeln. Trefferquote, R, Profitfaktor und Drawdown erst ab mindestens 20 eindeutigen Ergebnissen je Klasse vergleichen; keine automatische Gewichtung oder Zielquote.
...
- Vollständiger realer Lauf des breiten 2.520-Asset-Research-Datensatzes. Code, 248/248-Auditgate, append-only Übergangsnachweis und blockweiser Autostart sind fertig und getestet; der Start bleibt bis zum unveränderten Kampagnenabschluss gesperrt. Daher existieren noch keine belastbaren Kandidatenzahlen oder Development-Musterergebnisse.
...
- Reale Freigabe irgendeines technischen Challengers. Der manuelle C-Freeze sowie der Ground-up-Pfad Validation → Holdout → External → True Forward sind technisch gesperrt vorbereitet, aber mangels abgeschlossenem Broad-Pass existiert noch kein C-Kandidat. Kein RSI-, EMA-, Kombinations- oder Setup-Challenger hat die Long-v1-Produktionsbasis ersetzt.
...
- Mehrtägiger Realnachweis aller vier regionalen Swing-Hintergrundaufgaben. Der erste Europa-Lauf und zwei sichere Nachläufe sind belegt; Amerika/Global, Krypto und Asien/Australien sowie mehrere Marktphasen und echte abgeschlossene Paper-Ergebnisse stehen noch aus.
...
- Broker-Anbindung, Orderausführung und automatische Käufe oder Verkäufe sind bewusst nicht vorgesehen.
...
- Die technische Challenger-Kampagne ist mit zuletzt 243/248 Jobs noch nicht vollständig. Solange die fünf verbliebenen C-Jobs und der abschließende Robustheitsvergleich fehlen, darf aus Teilkennzahlen weder ein Sieger noch eine Produktionsänderung abgeleitet werden.
- Der autonome Paper-Bot besitzt erst drei gespeicherte Signale; die Stichprobe ist für Trefferquote, Rendite, Drawdown oder Strategieverbesserung nicht belastbar. Shadow-Live besitzt drei Orderentwürfe, aber noch keine echten Bid-/Ask-/Fill-Beobachtungen. Ohne belastbare Quelle werden diese Daten bewusst nicht geschätzt.
